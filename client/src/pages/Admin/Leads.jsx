...
const Leads = () => {
  ...
  //handleChange for customer and employee
  ...
  useEffect(() => {
    const lC = async () => {
      try {
        const [leadRes, customerRes, employeeRes] = await Promise.all([
          api.get("/admin/leads"),
        ...
        const employeesData = employeeRes.data.data.filter(
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      assignedTo: form.assignedTo || null, // Ensure assignedTo is null if not provided
      customer: form.customer || null,
    };

    try {
      if (editLead) {
        const response = await api.put(`/admin/lead/${editLead._id}`, payload);

        const updatedLead = response.data.data;
        setLeads((prev) =>
          prev.map((l) => (l._id === editLead._id ? updatedLead : l)),
        );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this lead?")) {
      try {
        ...
        toast.error(
          error.response?.data?.data?.message || "Failed to delete lead",
        );
      }
    }
  };
  ...
  const statusColors = {
    ...
    contacted:
      "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400",
    qualified:
      "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-500/20 text-amber-400",
    converted:
      "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-500/20 text-green-400",
      ...
      render: (row) => (
        <span
          className={
            ...
            "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-500/20 text-orange-400"
          }
        >
          {row.status}
        </span>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={openAdd} icon={<FiPlus size={16} />}>
          Add Lead
        </Button>
      </div>
      {/*pipeline summary*/}
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {["new", "contacted", "qualified", "converted", "lost"].map(
          (status) => (
            <div
              key={status}
              className="bg-[#1a1a1a]/80 backdrop-blur-xl border border-[#2a2a2a]/50 rounded-xl p-3 text-center"
            >
              <p className="text-xl font-bold text-white">
                {leads.filter((l) => l.status === status).length}
              </p>
              <p
                className={`text-xs font-medium capitalize mt-1 ${statusColors[status]?.replace("badge-", "text-") || "text-gray-400"}`}
              >
                {status}
              </p>
            </div>
